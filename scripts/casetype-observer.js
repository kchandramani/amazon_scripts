...
Copy CodeATTRIBUTES: '/api/getSourceAddressAttributes',
...
Copy Code{ match: 'PBG',         color: 'rgba(233, 69, 96, 0.9)'  },
...
Copy Code'CUSTOMER', 'LLM', 'AMZL', 'POSTAL', 'CAIMS',
...
Copy Codeif (!src) return 'rgba(233, 69, 96, 0.9)';
...
Copy Codeif (!src) return { label: 'UNKNOWN', type: 'other' };
...
const PANEL_CSS = ` #gs-panel { position: fixed; width: 380px; max-height: 80vh; background: #1a1a2e; color: #e0e0e0; border: 2px solid #00d4ff; border-radius: 10px; z-index: 999999; font-family: 'Segoe UI', Arial, sans-serif; font-size: 13px; box-shadow: 0 6px 24px rgba(0, 212, 255, 0.25); overflow: hidden; transition: border-color 0.5s; } #gs-panel.minimized #gs-body { display: none; } #gs-panel.minimized { width: 200px; }
...
Copy Codereturn val ? val.replace(/^"|"\$/g, '').trim() : 'N/A';
}

function fmtDate(d) {

Copy Codeif (!d) return '';
...
Copy Codeconst ta = new Date(a.formattedDateTime || a.timestamp || 0).getTime();
...
Copy Codeconst info = cleanSourceName(src);
...
Copy Codeconst url = `https://www.bing.com/translator/?text=${encodeURIComponent(text)}&from=auto&to=en`;
...
Copy Codefor (const el of $$(selector)) {
...
Copy Codeif (attr.aidDetails && attr.aidDetails.length > 0) {
...
Copy Codeif (state.interceptorsSetup) return;
...
Copy Codeif (!state.active || !data) return;
...
Copy Codeif (!state.active || !state.waitingForData) return;
...
Copy CodedestroyFloatingDisplay();
...
Copy Codeif (state.floatingDisplay) {
...
Copy Codeif (!state.active) return;
...
Copy Codeif (!state.active || state.textFound || state.isChecking) return;
...
Copy Codeif (state.textFound) return;
...
Copy Codeif (!state.active || state.buttonClicked) return;
...
Copy Codeif (!state.active || state.sharedDeliveryClicked || retry >= MAX_RETRIES) return;
...
Copy Codeif (!state.active || state.editDetailsClicked || retry >= MAX_RETRIES) return;
...
Copy Codeconst acc = findAttributesAccordion();
...
Copy CodefindByText('[role="button"]', 'Past deliveries') ||

findByText('.MuiAccordionSummary-root', 'Past deliveries');
if (accordion) {

Copy Codeif (accordion.getAttribute('aria-expanded') !== 'true') {
...
Copy Codeif (state.userDragged) return;
...
Copy Codeconst header = panel.querySelector('#gs-header');
...
Copy Codeconsole.log('[GS Panel] 🧹 Full cleanup started');
...
Copy Codeconsole.log('[GS Panel] 🔄 Ready for next case');
...
Copy Codeconsole.log('[GS Panel] Initializing (Memory Optimized + Live Detection)...');
...
Copy Codedocument.addEventListener('DOMContentLoaded', initialize);
} else {
