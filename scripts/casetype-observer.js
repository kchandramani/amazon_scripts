// CaseType Text Observer + GS Panel (DH & PDL via Intercept)
...
(function() {
    ...
    function cleanSourceName(src) {
        ...
        if (s.includes('CUSTOMER')) {
            return { label: 'CUSTOMER', type: 'customer' };
        }

        // LLM variants
        if (s.includes('LLM')) {
            return { label: 'LLM', type: 'llm' };
        }

        // AMZL variants — covers:
        // PROPAGATED.BUILDING.AMZL DELIVERY STATION
        // PROPAGATED.CAMPUS.AMZL_DELIVERY_STATION
        // AMZL_DELIVERY_STATION
        // AMZL etc.
        if (s.includes('AMZL')) {
            return { label: 'AMZL', type: 'amzl' };
        }

        // POSTAL_AUTHORITY
        if (s.includes('POSTAL')) {
            return { label: 'POSTAL', type: 'postal' };
        }

        // CAIMS_DERIVED
        if (s.includes('CAIMS')) {
            return { label: 'CAIMS', type: 'caims' };
        }

        // DERIVED
        if (s.includes('DERIVED')) {
            return { label: 'DERIVED', type: 'derived' };
        }

        // REGRID
        if (s.includes('REGRID')) {
            return { label: 'REGRID', type: 'regrid' };
        }

        // RTS
        if (s.includes('RTS')) {
            return { label: 'RTS', type: 'rts' };
        }

        // SDS
        if (s.includes('SDS')) {
            return { label: 'SDS', type: 'sds' };
        }

        // TRANSPORTER
        if (s.includes('TRANSPORTER')) {
            return { label: 'TRANSPORTER', type: 'transporter' };
        }
    ...
    function setupInterceptors() {
        ...
        window.fetch = async function(...args) {
            ...
            try {
                ...
                if (url.includes(API_PATH) && waitingForData) {
        ...
        XMLHttpRequest.prototype.send = function(...args) {
            this.addEventListener('load', function() {
                try {
                    if (this._interceptUrl && this._interceptUrl.includes(API_PATH) && waitingForData) {
    ...
    function handleDataReceived(data) {
        ...
        console.log('[GS Panel] Data received, parsing DH & PDL...');
        ...
        renderPanel(dhEntries, pdlEntries);
    ...
    // ==================== RENDER PANEL ====================
    function renderPanel(dhEntries, pdlEntries) {
        const body = document.getElementById('gs-body');
        if (!body) return;
        ...
        if (dhEntries.length > 0) {
            dhEntries.forEach((entry, i) => {
                ...
                html += `<div class="gs-entry-num">#${i + 1}</div>`;
        ...
        // ---- Preferred Delivery Locations (ALL) ----
        html += '<div class="gs-block pdl">';
        html += `<div class="gs-label">🟢 Preferred Delivery Locations (${pdlEntries.length})</div>`;
        if (pdlEntries.length > 0) {
            pdlEntries.forEach((entry, i) => {
                if (i > 0) html += '<hr class="gs-entry-divider">';
                html += '<div class="gs-entry">';
                html += `<div class="gs-entry-num">#${i + 1}</div>`;
                html += `<div class="gs-val">${parsePDL(entry.value)}</div>`;
                html += `<div class="gs-meta">${srcBadge(entry.attributeSrc)} <span class="gs-sep">|</span> <span class="gs-date">${fmtDate(entry.formattedDateTime)}</span></div>`;
                html += '</div>';
            });
        } else {
    ...
    // ==================== INITIALIZE ====================
    function initialize() {
        console.log('[GS Panel] Initializing v3.1 (intercept + color sources)...');
