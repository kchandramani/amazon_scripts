...
(function () {
    ...
    const Logger = {
        ...
        info: function (msg) {
            console.log(`%c${this.prefix} ${msg}`, `color: ${IS_IFRAME ? '#2196F3' : '#FF9900'}; font-weight: bold;`);
        },
        warn: function (msg) {
    ...
    // ╔══════════════════════════════════════════════════════════╗
    // ║  LOADING OVERLAY (Only on parent page, not iframe)       ║
    // ╚══════════════════════════════════════════════════════════╝
    ...
    async function main() {
        ...
        if (IS_IFRAME) {
            Logger.info('📦 Running INSIDE IFRAME');
            if (IS_TEMPLATE_PAGE) Logger.info('📍 Detected: Template Page (inside iframe)');
        } else {
            if (IS_PLACE_PAGE) Logger.info('📍 Detected: Place Page (parent)');
        ...
        if (Cache.isFirstRun()) {
            if (IS_IFRAME) {
                // Iframe: wait for parent to download first
                Logger.info('⏳ First run inside iframe - waiting for cache...');
                waitForCacheThenLoad();
                return;
            }
        ...
        if (!manifest) {
            if (IS_IFRAME) {
                Logger.warn('⚠️ No cache in iframe. Waiting...');
                waitForCacheThenLoad();
                return;
            }
        ...
        // Load scripts matching current page
        loadScriptsFromCache(manifest);

        // ────────────────────────────────────────
        // CASE 3: Check for updates (only from parent, not iframe)
        // ────────────────────────────────────────
        if (!IS_IFRAME) {
    ...
    function waitForCacheThenLoad() {
        ...
        const checkInterval = setInterval(function () {
            ...
            if (manifest) {
                ...
                Logger.info('✅ Cache ready! Loading scripts...');
                loadScriptsFromCache(manifest);
                return;
            }
            ...
            if (attempts >= maxAttempts) {
                clearInterval(checkInterval);
                Logger.warn('⚠️ Timeout waiting for cache. Trying to download directly...');
                firstTimeDownload();
            }
        }, 100);
    }
    ...
    // ╔══════════════════════════════════════════════════════════╗
    // ║  TAMPERMONKEY MENU (Only on parent page, not iframe)     ║
    // ╚══════════════════════════════════════════════════════════╝
    ...
    if (!IS_IFRAME) {
        ...
        GM_registerMenuCommand('📋 Script Status', function () {
            ...
            manifest.scripts.forEach(function (sc) {
                ...
                const matches = shouldRunOnCurrentPage(sc.matchPatterns) ? '🟢 parent' : '🔴';
                const matchesTemplate = sc.matchPatterns && sc.matchPatterns.some(function(p) {
                    return p.includes('templates');
                }) ? ' 📦 iframe' : '';
                s += enabled + ' ' + cached + ' ' + matches + matchesTemplate + ' [P' + sc.priority + '] ' + sc.name + '\n';
                s += '   📝 ' + sc.description + '\n\n';
            });

            s += '────── Legend ──────\n';
            s += '✅/❌ = Enabled/Disabled | 💾/⚠️ = Cached/Not cached\n';
            s += '🟢 = Runs on parent page | 📦 = Runs in template iframe\n';
